...
abstract class Pop3Handler {
    ...
    private void doUSER(String username) throws Pop3CmdException, IOException {
        ...
        // SYNC - Untl we migrate to zmnginx, use hacky admin auth
        if (username.contains(USER_SEPARATOR)) {
            String[] users = username.split(USER_SEPARATOR);
            username = users[0];
            authUser = users[1];
        }
    ...
    private void doPASS(String password) throws Pop3CmdException, IOException {
        ...
        // SYNC - authUser always exists now
        authenticate(user, authUser, password, null);
        sendOK("server ready");
    }
    ...
    void authenticate(String username, String authenticateId, String password, Authenticator auth)
        ...
        try {
            // SYNC - We're always passing an authenticateId for this client
            ...
            // SYNC - update authorization account logon timestamp:
            Provisioning.getInstance().accountAuthed(acct);
